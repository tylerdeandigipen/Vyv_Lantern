...
int ImGuiManager::RenderDirPopup(const std::string& popupTitle, const std::string& message, const std::string& nameOfEntity, bool openPopup)
{
	int returnVal = 0;
	if (openPopup)
	{
	...
	if (strcmp(nameOfEntity.c_str(), "Mirror") == 0)
	{
		if (ImGui::BeginPopupModal(popupTitle.c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			ImGui::Text(message.c_str());
	...
	else if (strcmp(nameOfEntity.c_str(), "Emitter") == 0)
	{
		if (ImGui::BeginPopupModal(popupTitle.c_str(), NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			ImGui::Text(message.c_str());
		...
		{
			assert("Passed in name is not Emitter or Mirror!");
		}
	}
}
