...
Logging& logger_ = Logging::GetInstance("debugLog_Laser.log");

//ImageBuffer* testSprite;
//ImageBuffer* testSprite1;
//ImageBuffer* topwall_, * bottomwall_, * leftwall_, * rightwall_;
...
SDL_Window* window_;
Renderer* pixelRenderer_ = Renderer::GetInstance();

bool ableMove = true; // Initialize to allow movement

//SDL_GLContext glContext_; // OpenGL context for SDL2

Scene* TLSSceneinstance = NULL; // ITS A GLOBAL VARIABLE CALM DOWN!! SHOW ME ANOTHER WAY AND ITS GONE
//
...
static bool TSshow_demo_window_ = false;
static bool TSshow_tool_metrics = false;
static bool TSshow_custom_window_ = true;
...
/* NOTICE !!!!!!!!!! Feel free to "turn off" debug messages as you please. You can see them in the debugLog.txt in the game files, or in the output tab when debugging. Literally
   you can call for the logger_ anywhere as long as you get an instance first. Use it in ur functions or something - taylee */

TLSScene::TLSScene() : Scene("test")
{

}

Engine::EngineCode TLSScene::Load()
{

    return Engine::NothingBad;
}

Engine::EngineCode TLSScene::Init()
{
    {
    ...
    logger_.LogToAll("Timestamped message: %s", "Starting TLSScene!!");

    Inputs::GetInstance()->SetWindow(window_);
    ...
    // Create SDL window_
    window_ = PlatformSystem::GetInstance()->GetWindowHandle();
    pixelRenderer_->window = window_;

    logger_.LogLine("Debug info: Lights, camera, action! (TLSScene init)");
    return Engine::NothingBad;
}

int ablePlaceMoreLight = 0;
float moveSpeed_ = 20;
...
//        {
//            pixelRenderer_->objects[2]->position.y -= moveSpeed_ * dt;
//
//            logger_.LogLine("Debug info: Vyv Up pressed.");
//            //pixelRenderer_.AddLight(pixelRenderer_.staticLightSource[0]);
...
//            pixelRenderer_->objects[2]->position.y += moveSpeed_ * dt;
//
//            logger_.LogLine("Debug info: Vyv Down pressed.");
//            //AudioManager.PlaySFX("footsteps.ogg");
//        }
//        if (inputHandler->keyPressed(SDL_SCANCODE_D))
//        {
//            pixelRenderer_->objects[2]->position.x += moveSpeed_ * dt;
//
//            logger_.LogLine("Debug info: Vyv Right pressed.");
//            //AudioManager.PlaySFX("footsteps.ogg");
//        }
...
//            pixelRenderer_->objects[2]->position.x -= moveSpeed_ * dt;
//
//            logger_.LogLine("Debug info: Vyv Left pressed.");
//            //AudioManager.PlaySFX("footsteps.ogg");
//        }
//    }
//
//    if (inputHandler->keyPressed(SDL_SCANCODE_E) && ablePlaceMoreLight == 1)
//    {
//        pixelRenderer_->AddLight(pixelRenderer_->lightSource[0]);
//        ablePlaceMoreLight = 0;
//    }
//
//    if (!inputHandler->keyPressed(SDL_SCANCODE_E))
//    {
//        ablePlaceMoreLight = 1;
//    }
//
...
//        {
//            TSshow_demo_window_ = !TSshow_demo_window_;
//            TSshow_tool_metrics = !TSshow_tool_metrics;
//            TSshow_custom_window_ = !TSshow_custom_window_;
//
//        }
...
//    CursourP *= 1.0f / pixelRenderer_->screenScale;
//    CursourP += pixelRenderer_->GetCameraPosition();
//
//    Vector2 LightP = pixelRenderer_->lightSource[0].position;
//    Vector2 D = LightP - CursourP;
//    float Angle = atan2f(D.x, D.y) * (180.0f / 3.14f) + 180.0f;
//    // pixelRenderer_->lightSource[0].angle = Angle;
//
//    ImageBuffer* playerEntity = pixelRenderer_->objects[1];
//    Vector2 ScreenHalfSize = 0.5f * Vector2(SCREEN_SIZE_X, SCREEN_SIZE_Y);
//    Vector2 BitmapHalfDim = 0.5f * playerEntity->size;
//    pixelRenderer_->SetCameraPosition(playerEntity->position - ScreenHalfSize + BitmapHalfDim);
//}

void TLSScene::Update(float dt)
{
    ...
    pixelRenderer_->Update(dt);
}

void TLSScene::Render()
{
#ifdef _DEBUG
    ImGuiInterg();
#endif
...
Engine::EngineCode TLSScene::Exit()
{
    // Remember to clean up
    Inputs::GetInstance()->InputKeyClear();
    pixelRenderer_->CleanRenderer();

    logger_.LogLine("Debug info: Bye-Bye! (TLSScene exit)");
    return Engine::NothingBad;
}

Engine::EngineCode TLSScene::Unload()
{
    delete TLSSceneinstance;
    TLSSceneinstance = nullptr;

    LevelBuilder::GetInstance()->FreeLevel();

    logger_.LogLine("Debug info: entities destroyed :( (TLSScene unloaded)");
    return Engine::NothingBad;
}

Scene* TLSSceneGetInstance(void)
{
    static Scene* TLSSceneinstance = nullptr; // Make it static to ensure a single instance
    if (!TLSSceneinstance) {
        TLSSceneinstance = new TLSScene();
    }
    return TLSSceneinstance;
}

/**********************************************************************/
void TLSScene::ImGuiInterg()
{
    ImGui_ImplOpenGL3_NewFrame();
    ImGui_ImplSDL2_NewFrame();
    ImGui::NewFrame();

    if (TSshow_custom_window_)
    {
#ifdef _DEBUG
        ImGuiWindow();
#endif
...
void TLSScene::ImGuiWindow()
{
    if (TSshow_custom_window_)
    {
        ImGui::Begin("custom window_");
        ...
        Particle** particles = pixelRenderer_->particleManager->GetParticles();
